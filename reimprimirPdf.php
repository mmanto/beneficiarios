...
$nro_Resolucion = $_POST['nroResolucion'];
$resol = split("/", trim($nro_Resolucion));
$resolucion_solo_nro = $resol[0];
$resolucion_solo_anio = $resol[1];



//TODO necesito $partido (elID), $barrio (elId), $idResolucion
$qryDatos = "SELECT idResolucion, Resolucion_partido, Resolucion_barrio_nro 
				FROM dbo_resolucion WHERE  Resolucion_nombre LIKE '$nro_Resolucion'";
$rs_datos = armar_matriz($qryDatos);
if(! is_array($rs_datos)){
	echo "No existe el numero de resolucion ingresado";
	echo '<h4><a href="javascript:history.go(-1)">Volver al formulario</a></h4>';
	exit();
}

$partido = $rs_datos[0]['Resolucion_partido'];
$barrio = $rs_datos[0]['Resolucion_barrio_nro'];
...
<table width="600" border="0" cellpadding="0" cellspacing="0">
	<tr>
	  <td height="30"><h2>Reimprimir Certificados de Adjudicación o Boletos de Compraventa</h2></td>
  </tr>
	<tr>
...
<input readonly="readonly" size="4" type="text" name="nro_Resolucion" id="nro_Resolucion" value="<?php echo $resolucion_solo_nro?>">
/
<input readonly="readonly" size="4"  type="text" id="nro_Resolucion_anio" name="nro_Resolucion_anio" value="<?php echo $resolucion_solo_anio?>">
