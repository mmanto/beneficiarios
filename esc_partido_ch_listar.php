...
$sql = "SELECT * FROM dbo_lote WHERE Partido_nro = $idPartido AND Lote_chacra = '$chacra' AND Lote_matricula != '0' AND blnActivo = '1'  ORDER BY Lote_manzana ASC, Lote_parcela ASC";
$res = mysql_query($sql);
$cant = mysql_num_rows($res);

}else{

$sql = "SELECT * FROM dbo_lote WHERE Partido_nro = $idPartido AND Lote_chacra = '$chacra' AND blnActivo = '1'  ORDER BY Lote_manzana ASC, Lote_parcela ASC";
$res = mysql_query($sql);
$cant = mysql_num_rows($res);
...
if ($cant < 1) { ?><h2>No hay registros | <?=$idPartido;?> - <?=$chacra; ?> | <?=$blnEsc; ?></h2><p><a href="javascript:history.go(-1)">Volver</a></p><p>&nbsp;</p> <? }else{

?><table border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="9" colspan="2"><h2>Listado beneficiarios en <? echo $partido_nombre ?> (Chacra <?=$chacra; ?>) - <?=$cant; ?> resultados </h2></td>
  </tr>
  <tr>
    <td height="30" colspan="2"><a href="javascript:history.go(-1)">Volver</a></td>
  </tr>
  <tr>
    <td width="10">&nbsp;</td>
