...
$sql = "SELECT Familia_nro, Familia_apellido, Familia_res_adj, Familia_doc_completa, Familia_pagocancelado, Familia_cond_escrit, Familia_tramitependiente, Expte_esc_nro, Lote_manzana_prov, Familia_matricula, Familia_observaciones,  Nombre FROM (
dbo_familia
INNER JOIN
dbo_usuarios
...
$cantit = mysql_num_rows($sql2);

?>

       <? echo $manzana_prov; ?>;<? echo $manzana; ?>;<? echo $parcela; ?>;<? echo $cantit; ?>;<? while ($persona = mysql_fetch_array($sql2)){ ?><? echo $persona["Persona_apellido"].", ".$persona["Persona_nombre"]; ?> (<? echo $persona["Persona_dni_nro"]; ?>) / <? } ?>;<?=$familia["Familia_res_adj"] ?>;<?=$familia["Familia_observaciones"] ?><br/>
        </tr>
  <?
}
?>
